...
软件架构就像房子的设计图：不是画具体怎么砌砖，而是画房间怎么布局、每个房间干什么用、房间之间怎么连接。架构关注的是**概念**和**整体结构**，不是实现细节。模块化就像把大房子分成小房间，每个房间有明确的功能（客厅会客、卧室睡觉），还要考虑质量要求（客厅要通风、卧室要安静），以及约束（房间之间要有门连接）。
...
架构风格就像"盖房子的不同方式"：**分层**就像盖楼，一层一层，每层有明确职责（比如网络7层模型）。**MVC**就像把系统分成三部分：模型（数据）、视图（界面）、控制器（逻辑），互不干扰。**客户端-服务器**就像餐厅，客户点菜，厨房做菜。**事件总线**就像广播，发布者发消息，订阅者收消息。**管道-过滤器**就像流水线，数据流过一个一个处理器。**微内核**就像插件系统，核心很小，功能靠插件扩展。

设计模式就像"常见问题的标准解法"：Observer（观察者）就像"订阅通知"，Visitor（访问者）就像"访问者模式"。企业集成模式就像"连接不同系统的标准方式"。

策略就是"谁负责什么质量"：View负责易用性（界面要好看好用），Controller负责互操作（要能和其他系统对接），并设定约束（Controller的改动不能影响View）。
...
设计细化就像"逐步完善"：一开始需求不明确，架构也粗糙。随着需求明确，逐步细化架构。比如发现需要安全性，就在UI层加认证；发现需要高可用，就在数据层加冗余（备份）；发现需要高正确性，就设计成可测试的，或者用N版本编程（多个版本同时运行，对比结果）。就像盖房子，先搭框架，再逐步完善细节。
...
实践要点就像"架构设计的原则"：**按职责和质量分解**，不是只按功能（比如"登录功能"），还要考虑质量（比如"登录要安全、要快"）。**分离关注点**，UI和业务逻辑分开，改UI不影响业务逻辑。**记录质量属性和约束**，说明每个模块负责什么质量，有什么约束。架构文档关注"做什么、为什么这样做"，不关注"具体怎么实现"（那是代码的事）。就像建筑设计图，画房间布局、功能，不画具体怎么砌砖。
...
Parnas的提示就像"分解方式影响很大"：功能分解（按功能分）和面向对象分解（按对象分），对变更的影响范围差别很大。信息隐藏（隐藏实现细节）和完整性（模块完整）是关键。就像整理东西，按功能分（所有工具放一起）和按对象分（每个东西的工具放一起），改一个东西的影响范围完全不同。
...
架构关注点就像"架构文档写什么"：架构关注重要的质量属性（比如性能、安全）、演化原则（以后怎么扩展）、关键场景（典型使用场景），不关注实现细节（具体怎么编码）。就像建筑设计图，画房间布局、功能分区，不画具体怎么砌砖。
