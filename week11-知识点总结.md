...
软件维护就像汽车保养：买车只是开始，后续的保养、维修、升级才是大头。软件也一样：开发可能只占30-40%的成本，维护要占60-70%。因为软件交付后还要：修bug、加功能、适应新环境（比如新操作系统）、优化性能等等。就像房子，盖好只是开始，后续的装修、维修、改造才是长期成本。
...
维护的原因就像"为什么要维护"：**修bug**，发现错误要修。**需求变化**，用户要新功能。**环境变化**，操作系统升级、硬件升级，软件要适配。**提升效率**，优化性能。**优化**，改进代码。**消除不良影响**，去掉不好的功能。就像汽车，要修故障、要升级、要保养、要优化，不是买来就不管了。
...
**适应性维护**：环境变了，软件要适应。比如Windows升级了，软件要适配新系统。就像手机系统升级，APP要更新才能用。

**完善性维护**：加新功能或改进。比如原来只能发文字，现在要能发图片。就像手机APP不断更新，加新功能。

**纠错性维护**：修bug。比如发现支付功能有问题，要修复。就像汽车出故障要修。

**预防性维护**：提前预防问题。比如发现代码有隐患，提前重构。就像定期保养汽车，防止出大问题。
...
维护流程就像修东西：先搞清楚要修什么（确定目标），然后了解这东西怎么工作的（理解程序），想好怎么修（准备方案），让别人看看方案对不对（评审），开始修（实施变更），最后测试一下修好了没（验证）。在敏捷开发中，这个流程就融入到日常开发任务中，每次改代码都走这个流程。
...
维护的挑战就像"修别人的东西很难"：**维护他人代码**，不知道别人怎么想的，很难理解。**理解不足**，代码复杂，看不懂。**结构欠佳**，代码写得乱，难改。**人员流动快**，写代码的人走了，没人知道为什么这样写。**缺少上下文**，不知道当时为什么这样设计。**系统未为变更而设计**，系统设计时没考虑以后要改，改起来很困难。就像修别人的车，不知道他怎么改装的，很难修。
...
版本管理就像写作文的版本控制：每次修改都保存一个版本，可以看历史、回退、合并。**代码线**就像作文的不同版本（v1, v2, v3...），**基线**就像正式发布的版本（比如"最终版"）。**分支**就像同时写两个版本（一个正式版，一个实验版），**合并**就是把两个版本合起来。Git会智能存储，只存变化的部分，节省空间。CI/CD需要版本管理，因为要能随时获取某个版本进行自动构建和测试。
...
可测性就像"容易检查"：**可控性**，能控制输入和执行（比如能设置测试数据、能控制执行流程）。**可观测性**，能观察输出和状态（比如能看到结果、能看到内部状态）。策略：用专用接口（测试接口）、录制/回放（记录操作，重复执行）、抽象数据源（用假数据）、沙箱（隔离环境）、自检断言（自动判断对错）。还要降低复杂度（减少依赖、减少循环、减少不确定性）。就像体检，要能控制检查什么（可控性），要能看到结果（可观测性），还要简单易操作。
