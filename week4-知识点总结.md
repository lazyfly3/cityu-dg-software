...
**功能需求**就像"我要一个能买PS5的网站"，这是具体的行为，可以验证。**非功能需求**就像"网站要快、要安全、要好看"，这些是质量要求，可能很模糊。比如"要快"多快？需要具体化，比如"1秒内打开页面"。非功能需求在不同层面都有：整个行业的要求（比如金融系统必须安全）、这类应用的要求（比如电商网站要能处理高并发）、这个具体应用的要求（比如这个网站要支持1000人同时在线）。
...
需求文档常见问题就像写作文时的各种错误：**噪声**是无关信息（比如写"用户要买PS5，顺便提一下天气不错"），**沉默**是漏掉重要信息（比如没说怎么付款），**过度规范**是直接说怎么做而不是要什么（比如说"用MySQL数据库"而不是说"要存数据"），**矛盾**是前后不一致（比如前面说"免费"后面说"收费"），**模糊**是不清楚（比如"要快"多快？），**空想**是不切实际（比如"要能预测未来"）。自然语言容易有这些问题，所以需要验证和评审。
...
需求获取就像做市场调研：要了解用户真正想要什么。**访谈**就像面对面聊天，适合新项目，可以深入挖掘；**头脑风暴**就像大家一起想点子；**任务分析**就像把"做一顿饭"分解成"买菜、洗菜、切菜、炒菜"；**场景分析**就像观察用户实际怎么用产品；**焦点小组**就像找几个典型用户一起讨论；**引导式研讨会**就像开工作坊，大家一起研究。问卷通常效果不好，因为用户可能不理解问题，或者懒得认真填。
...
概念建模就像画地图：把业务领域的概念、关系画出来，让大家对"我们在做什么"有共同理解。比如做电商系统，要明确"用户"、"订单"、"商品"这些概念是什么意思，它们之间什么关系。这样可以减少误解，避免后期发现"你说的用户和我说的用户不是一回事"这种问题。分析员要找出模糊的地方并澄清，还要协调不同利益相关者的冲突（比如销售想要简单，技术想要安全）。
...
分析员立场就像"看问题的角度"：**功能主义**（客观+秩序），分析员是专家，客观分析，冲突由管理层解决。就像医生看病，客观诊断，治疗方案由医院决定。

**社会相对主义**（主观+秩序），分析员是变革者，追求共识。就像调解员，协调各方，达成一致。

**激进结构主义**（客观+冲突），在冲突中选边站。就像律师，代表一方，对抗另一方。

**新人文主义**（主观+冲突），社会治疗者，消除壁垒。就像心理咨询师，帮助各方理解彼此，消除隔阂。

不同立场用不同方法，没有对错，看情况选择。
...
良好实践就像"做需求的标准流程"：**尽早验证**，不要等做完了才发现理解错了，就像做菜前先确认要做什么菜。**功能和非功能都要**，既要"能做什么"（功能），也要"做得多好"（非功能），而且要可测量，不能只说"要快"，要说"1秒内"。**用领域模型和术语**，大家用同样的词，避免"你说的用户和我说的用户不是一回事"。**记录并协商冲突**，不同人有不同需求，要记录、要协商，不能忽略。就像点菜，要记录每个人的需求，有冲突要协商。
...
非功能需求术语就像"同一个东西的不同叫法"：非功能需求也叫质量属性、系统特性、约束、concerns、goals等。从行业层面→应用类型层面→具体场景层面，层层细化。就像"车"在不同语境下叫"汽车"、"轿车"、"代步工具"等，但都是指同一个东西。
...
需求规约常见问题就像"写需求容易犯的错误"：**噪声**，无关信息太多。**沉默**，该说的没说。**过度规范**，说得太细，限制太死。**矛盾**，前后不一致。**模糊**，不清楚。**前向引用**，引用还没定义的东西。**空想**，不切实际。自然语言写需求风险高，容易有这些问题，所以要验证和评审。就像写作文，容易写错、写漏、写矛盾，要检查。
